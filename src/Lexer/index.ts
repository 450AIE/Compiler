...
class Lexer {
  static parse(codeSource: string) {
    ...
    while (iterator.hasNext()) {
       ...
       */
      const { value: char } = iterator.nextSourceChar();
      iterator.putBack();
      if (char === TokenType.EOF) break;
      // 空格，换行就略过，并且吃掉
      if ([" ", "\n"].includes(char)) {
        iterator.nextSourceChar();
        continue;
      }
      // 括号
      if (["(", ")", "[", "]", "{", "}"].includes(char)) {
        const bracketChar = iterator.nextSourceChar();
        tokens.push(
          new Token({
        ...
        const punctuationChar = iterator.nextSourceChar();
        tokens.push(
          new Token({
            type: TokenType.PUNCTUATION,
            value: char,
            loc: punctuationChar.loc,
          }),
        );
      } else {
